...
tfidf = TfidfVectorizer(norm = 'l2', use_idf = True)#逆向文件频率  #先映射再降维
df1 = tfidf.fit_transform(jieba_cut_content)
# print(df1.toarray())
...
nb = BernoulliNB(alpha = 1.0, binarize = 0.0005)#binarize二值转换阈值，alpha=1拉普拉斯平滑
model = nb.fit(data, y_train)

print('='*30 + '构建测试集' + '='*30)    
jieba_cut_content_test = list(x_test['jieba_cut_content'].astype('str'))
data_test = pd.DataFrame(svd.transform(tfidf.transform(jieba_cut_content_test)))
